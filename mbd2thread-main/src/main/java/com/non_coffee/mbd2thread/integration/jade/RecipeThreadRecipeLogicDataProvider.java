...
import com.non_coffee.mbd2thread.trait.RecipeThreadTrait;
...
public class RecipeThreadRecipeLogicDataProvider implements IServerDataProvider<BlockAccessor>, IBlockComponentProvider {
    private static final ResourceLocation UID = Objects.requireNonNull(ResourceLocation.tryParse("mbd2thread:recipe_thread_hud"));
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor blockAccessor, IPluginConfig config) {
        ...
        if (!data.contains("mbd2thread")) return;

        CompoundTag mbd2Thread = data.getCompound("mbd2thread");
        if (!mbd2Thread.contains("summary")) return;
        CompoundTag summary = mbd2Thread.getCompound("summary");
        String status = Objects.toString(summary.getString("status"), "IDLE");

        int running = mbd2Thread.getInt("runningThreads");
        int waiting = mbd2Thread.getInt("waitingThreads");
        int idle = mbd2Thread.getInt("idleThreads");
        ...
        if (!shift) {
            ...
            tooltip.add(Component.translatable("mbd2thread.jade.thread_summary", running, waiting, idle, base, progressText)
        ...
        if (mbd2Thread.contains("threads")) {
            ListTag threads = mbd2Thread.getList("threads", CompoundTag.TAG_COMPOUND);
            for (int i = 0; i < threads.size(); i++) {
                ...
                tooltip.add(Component.translatable("mbd2thread.jade.thread_line", index, statusText).withStyle(ChatFormatting.GRAY));
        ...
        if (mbd2Thread.contains("fuel")) {
            CompoundTag fuelTag = mbd2Thread.getCompound("fuel");
