...
public class LongFeEnergyDataProvider implements IServerDataProvider<BlockAccessor>, IBlockComponentProvider {
    private static final ResourceLocation UID = Objects.requireNonNull(ResourceLocation.tryParse("mbd2thread:long_fe_energy_hud"));
    ...
    public void appendServerData(CompoundTag data, BlockAccessor blockAccessor) {
        ...
        be.getCapability(LongFeEnergyCapability.CAPABILITY, null).resolve().ifPresent(container -> {
            ...
            data.put("mbd2thread_long_fe", tag);
        });
    }
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor blockAccessor, IPluginConfig config) {
        ...
        if (!data.contains("mbd2thread_long_fe")) return;

        CompoundTag tag = data.getCompound("mbd2thread_long_fe");
        ...
        tooltip.add(baseIndex + 1, Component.translatable(shift ? "mbd2thread.jade.long_fe_limits.raw" : "mbd2thread.jade.long_fe_limits", maxRText, maxEText)
