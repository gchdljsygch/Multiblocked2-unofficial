package com.non_coffee.mbd2thread.client.screen;

import com.lowdragmc.mbd2.common.item.MBDGadgetsItem;
import com.non_coffee.mbd2thread.Mbd2Thread;
import com.non_coffee.mbd2thread.network.C2SSetBuilderBuildModePacket;
import com.non_coffee.mbd2thread.network.C2SSetGadgetModePacket;
...
public class MbdGadgetModeWheelScreen extends Screen {
    ...
    public MbdGadgetModeWheelScreen() {
        super(Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.title"));
    }
    ...
    protected void init() {
        ...
        entries.add(Entry.mode(0,
                Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.mode.builder"),
                held.copyWithCount(1)));
        entries.add(Entry.mode(1,
                Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.mode.recipe_debugger"),
                held.copyWithCount(1)));
        entries.add(Entry.mode(2,
                Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.mode.multiblock_debugger"),
                held.copyWithCount(1)));
        ...
        if (BuilderMaterialBindings.isBuilder(held)) {
            ...
            entries.add(Entry.builderToggle(
                    Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.builder_mode"),
                    slowBuild,
                    held.copyWithCount(1)));
        }
    }
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (e.type == EntryType.BUILDER_TOGGLE) {
                ...
                Component state = e.builderSlow
                        ? Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.builder_mode.slow")
                        : Component.translatable("screen." + Mbd2Thread.MOD_ID + ".gadget_wheel.builder_mode.instant");
    ...
    private static void applySelection(Entry entry, InteractionHand hand) {
        if (entry.type == EntryType.MODE) {
            Mbd2ThreadNetwork.CHANNEL.sendToServer(new C2SSetGadgetModePacket(hand == InteractionHand.OFF_HAND ? 1 : 0, entry.modeDamage));
            return;
        }
        if (entry.type == EntryType.BUILDER_TOGGLE) {
            boolean next = !entry.builderSlow;
            Mbd2ThreadNetwork.CHANNEL.sendToServer(new C2SSetBuilderBuildModePacket(hand == InteractionHand.OFF_HAND ? 1 : 0, next));
        }
    }
