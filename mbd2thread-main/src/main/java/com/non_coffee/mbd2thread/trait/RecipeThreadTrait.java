...
public class RecipeThreadTrait implements ITrait {
    ...
    private void handleTickFailure(String stage, RuntimeException error) {
        if (!tickFailureLogged) {
            ...
            LOGGER.warn("[mbd2thread] {} failed for machine {} at {}; suppressing further reports for this trait. Likely caused by an upstream simulate-path bug (e.g. hotai transformer + ItemSlotCapabilityTrait). Original error: {}",
    ...
    public String getThreadStatusText(int threadId) {
        ...
        if (logic == null) return "mbd2thread.gui.thread_disabled";
